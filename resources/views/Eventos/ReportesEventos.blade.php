...
<div class="card" style="height: auto; width: auto;">
    ...
    <div class="container">
        @if (auth()->user()->hasRole('Personal Administrativo'))
        ...
        <br>
        @endif
        @if (auth()->user()->hasRole('Seguridad e Higiene')|| auth()->user()->email=='adejesus@intimark.com.mx'|| auth()->user()->email=='gvm7506@gmail.com')
                <br>
                <div class="container">
                    <div class="row">
                                        ...
                                        <select name="created_at" id="created_at" class="form-control" required>
                                            ...
                                            @foreach ($mesesSeleccionPapel as $mes)
                                                <option value="{{ $mes->fecha }}">
                                                    {{ \Carbon\Carbon::parse($mes->fecha)->translatedFormat('F - Y') }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col">
                        ...
                        <div class="col-md-6 card">
                            ...
                            <form method="POST" action="{{ route('eventos.GenerarReporteSimulacro') }}">
                                @csrf
                                <div class="form-row">
                                    <div class="col">
                                        <select name="created_at" id="created_at" class="form-control" required>
                                            <option value="">Selecciona un mes</option>
                                            @foreach ($mesesSeleccionSimulacro as $mes)
                                                <option value="{{ $mes->fecha }}">
                                                    {{ \Carbon\Carbon::parse($mes->fecha)->translatedFormat('F - Y') }}</option>
                                            @endforeach
                                        </select>
