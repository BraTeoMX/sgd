...
class FormatoPermisosController extends Controller
{
    ...
    public function revisarpermiso($id) //empleado entra a la empresa
    {
        ...
        if ($permiso != NULL) {
            ...
            $salida = FormatoPermisos::where('folio_per', $id)->update(['fech_fin_hor' => $hora, 'firmado' => 0, 'entrada_permiso' => 1]);
    ...
    public function revisarEntrada($folio) //empleado sale de la empresa
    {
        ...
        $permiso = FormatoPermisos::where('status', 'APLICADO')
            ...
            ->get()->first();
        if ($permiso != NULL) {
            $salida = FormatoPermisos::where('folio_per', $folio)->update(['fech_ini_hor' => $hora, 'firmado' => 0, 'salida_permiso' => 1]);
    ...
    public function duplicados(Request $request)
    {
       ...
       $duplicados = FormatoPermisos::where('fk_no_empleado', $request->empleado)
            ->where('fech_ini_per',  $request->dia)
            ->where('modo_per', 1)
