    ...
    public function store(Request $reg_sol_vac)
    {
        ...
        if ($reg_sol_vac->eventualidad == 1) {
         ...
         //   $data = array('no_empleado' => $vac->fk_no_empleado, 'eventualidad' => '1', 'periodo' => '0', 'anio' => '2023', 'fecha' => $reg_sol_vac->inicio_vac);
            $data = array('no_empleado' => $vac->fk_no_empleado, 'folio' => $vac->folio_vac,'eventualidad' => '1', 'periodo' => '0', 'anio' => $anio_actual, 'estatus' => 'APLICADO', 'fecha' => date('Y-m-d'),'inicial' => $reg_sol_vac->inicio_vac, 'final' => $reg_sol_vac->fin_vac);
      //adecuacion vacaciones 18 meses 2023
            DB::table('eventualidad_periodo')->insert($data);
        } else {
            $vac->periodos = '1';
            $data = array('no_empleado' => $vac->fk_no_empleado, 'folio' => $vac->folio_vac,'eventualidad' => '0', 'periodo' => '1', 'anio' => $anio_actual, 'estatus' => 'APLICADO', 'fecha' => date('Y-m-d'), 'inicial' => $reg_sol_vac->inicio_vac, 'final' => $reg_sol_vac->fin_vac);
//adecuacion vacaciones 18 meses 2023
...
/**************************** */
    ...
    public function cancelacion()
    {
        ...
        $vacaciones = Vacaciones::join('Tbl_Empleados_SIA', 'Tbl_Empleados_SIA.No_Empleado', '=', 'Vacaciones2.fk_no_empleado')
            ...
            ->whereMonth('fech_ini_vac','>=',$mes_actual)
            ->whereYear('fech_ini_vac','>=',$anio_actual)
...
//dd($eventualidad1);
    ...
    public function viewAjax(Request $request)
    {
        ...
        $nomina = Calendario::where('fecha_calendario', $fech)->orwhere('fecha_calendario', $fech2)->get()->first();

        if ($nomina->tipo_nomina == 3) {
            $datos = 1;
        } else {
            $datos = Calendario::where('fecha_calendario', $fech)->where('tipo_nomina', $frec_pago)->count();
            if($datos != 0)
              $datos = Calendario::where('fecha_calendario', $fech2)->where('tipo_nomina', $frec_pago)->count();
        }
    ...
    public function viewAjax3(Request $request)
    {
        ...
        if ($modulo == '') {
            ...
            if (ceil($porc_ausentismo) > $datos) {
             ...
             $datos = Vacaciones::join('Tbl_Empleados_SIA', 'Tbl_Empleados_SIA.No_Empleado', '=', 'Vacaciones2.fk_no_empleado')->select('No_Empleado')
