...
class ReportesEventosController extends Controller
{
    ...
    public function ReportesEventos(Request $request)
    {
        ...
        //Inicio de apartado  de la seccion del codigo implementado para la tabla de Simulacro
        $eventoSimulacro = RegistroSimulacro::all();
        // Obtiene los meses y años únicos de los registros
        $mesesAniosSimulacro = RegistroSimulacro::selectRaw("DATE_FORMAT(created_at, '%Y-%m') as fecha")
            ->distinct()
            ->get();
        $mesesSeleccionSimulacro = RegistroSimulacro::selectRaw("DATE_FORMAT(created_at, '%Y-%m') as fecha")
            ->distinct()
            ->get();

        // Inicializa un arreglo para los valores de selección de fechas
        $fechasSeleccionSimulacro = [];

        foreach ($mesesAniosSimulacro as $mesAnioSimulacro) {
            $fecha = \Carbon\Carbon::parse($mesAnioSimulacro->fecha);
            $fechasSeleccionSimulacro[$mesAnioSimulacro->fecha] = $fecha->format('F Y');
            //dd($fechasSeleccion);
        }

        // Obtiene la fecha seleccionada desde la solicitud
        $fechaSimulacro = $request->created_at;

        // Inicializa la consulta de reportes sin filtros de fecha
        $reportesQuerySimulacro = RegistroSimulacro::select('id', 'id_evento', 'no_empleados', 'No_Tag', 'tipo_evento', 'nombre_empleado', 'Puesto', 'created_at', 'asistencia', 'Planta', 'Departamento');

        // Si se seleccionó una fecha, filtra por ese rango
        if ($fechaSimulacro) {
            $reportesQuerySimulacro->whereDate('created_at', $fechaSimulacro);
        }

        // Obtiene los reportes de asistencia
        $reportesSimulacro = $reportesQuerySimulacro->get();

        $totalRegistrosSimulacro = $reportesSimulacro->count();
        ...
        return view('eventos.ReportesEventos', compact('eventos', 'reportes', 'totalRegistros', 'optionsave', 'nombre', 'fechasSeleccion', 'request','fecha','mesesSeleccion','mes',
                                                ...
                                                'reportesQueryPapel', 'reportesPapel', 'totalRegistrosPapel', 'mesesSeleccionSimulacro',));
    ...
    public function GenerarReportePapel(Request $request)
    {
        ...
        if($planta == "ambos"){
            ...
            ->get();
            //dd($reportesQuery);
        }else if($planta != "ambos")
        $reportesQuery = RegistroPapelTemporal::select('id', 'id_evento', 'no_empleados', 'No_Tag', 'tipo_evento', 'nombre_empleado', 'Puesto', 'created_at', 'asistencia', 'Planta', 'Departamento')
            ->whereYear('created_at', '=', substr($mes, 0, 4))
            ->whereMonth('created_at', '=', substr($mes, 5, 2))
            ->when($planta, function($query, $planta) {
        ...
        if ($request->has('excel')) {
         // dd($request->all());  // Redirige a la ruta de exportarExcel
            return redirect()->route('eventos.exportarExcelPapel', $request->all());
        }

        return $dompdf->stream($nombreArchivo);
    }
    public function exportarExcelPapel(Request $request)
    {
        $optionsave = 5;
        ...
        $nombre = RegistroPapelTemporal::where('id_evento', $optionsave)->value('tipo_evento');
...
$Asistencia = Tbl_Empleado_SIA::select('No_Empleado', 'Id_Planta', 'No_TAG', 'Nom_Emp', 'Ap_Pat', 'Ap_Mat','Puesto', 'Departamento','Fecha_In')
   ...
   // dd($RegistroIxtlahuacaTotal);
    ...
    public function GenerarReporteSimulacro(Request $request)
    {
        ...
        $nombre = 'Simulacro 2024';

        // Obtiene los meses y años únicos de los registros
        $mesesSeleccion = RegistroSimulacro::selectRaw("DATE_FORMAT(created_at, '%Y-%m') as fecha")
        ...
        // Inicializa la consulta de reportes sin filtros de fecha
        if ($planta == "ambos") {
            $reportesQuery = RegistroSimulacro::select('id', 'no_empleados', 'No_Tag', 'nombre_empleado', 'Puesto', 'created_at', 'Planta', 'Departamento')
                ->whereYear('created_at', '=', substr($mes, 0, 4))
                ->whereMonth('created_at', '=', substr($mes, 5, 2))
                ->get();
            //dd($reportesQuery);
        ...
        // Cambiar el encabezado de la columna "Asistencia" a "Rollos entregados" si la opción seleccionada es "5"
        $encabezadoColumna = "Reporte Entrega de Papel";
        ...
        // Renderiza la vista directamente
        $pdfView = view('eventos.PDFReportViewSimulacro', compact('datosReporte'))->render();
        ...
        if ($request->has('excel')) {
            // dd($request->all());  // Redirige a la ruta de exportarExcel
            return redirect()->route('eventos.exportarExcelSimulacro', $request->all());
        }

        return $dompdf->stream($nombreArchivo);
    }

    public function exportarExcelSimulacro(Request $request)
    {
        $fechaSeleccionada = $request->created_at;
        $mes = $request->input('created_at');
        // Lógica para obtener los datos y configurar la exportación
        $reportesQuery = RegistroSimulacro::select('id', 'no_empleados', 'No_Tag', 'tipo_evento', 'nombre_empleado', 'Puesto', 'created_at', 'Planta', 'Departamento')
            ->whereYear('created_at', '=', substr($mes, 0, 4))
            ->whereMonth('created_at', '=', substr($mes, 5, 2))
            ->get();
        //dd($reportesQuery);
